...
const saveSolution= async (solution,userId,problem_id,status) => {
    const response = await client.queryArray("INSERT INTO problems (problem_id, ifSaved, solution, user_id) VALUES ($1, $2, $3, $4)", [problem_id, status, solution, userId]);
    return response.rows;
};

const updateSolution=async (solution,userId,problem_id,status) => {
    const response = await client.queryArray("update problems set solution = $1, ifSaved=$4 where user_id=$2 and problem_id=$3",[solution,userId,problem_id,status]);
    return response.rows;
};
...
export { createUser, saveSolution, getAllSolutions, getUserId,submitSolution,updateSolution};
